...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pt-28">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 display-font">Manajemen Level</h1>
            <p class="text-slate-500 mt-1">Kelola data level akses user pada aplikasi</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            <!-- Stat Card -->
            <div
                class="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-3xl p-6 shadow-lg shadow-cyan-500/30 text-white relative overflow-hidden">
                <div class="absolute right-0 top-0 -mt-4 -mr-4 w-24 h-24 bg-white/10 rounded-full blur-xl"></div>
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-cyan-100 text-sm font-medium mb-1">Total Level Aktif</p>
                        <h3 class="text-4xl font-bold">{{ $totalLevels }}</h3>
                    </div>
            ...
            <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm">
                <div class="flex items-center mb-6 pb-4 border-b border-slate-100">
                    <div class="w-8 h-8 rounded-lg bg-cyan-100 text-cyan-600 flex items-center justify-center mr-3">
                        <i class="fa-solid fa-plus text-sm"></i>
                    </div>
                    <h2 class="text-lg font-bold text-slate-900">Tambah Level Baru</h2>
                </div>

                <form action="{{ route('store-level') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="namaLevel" class="block text-sm font-medium text-slate-700 mb-1">Nama Level</label>
                        <input type="text" id="namaLevel" name="namaLevel" required
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 focus:bg-white transition-colors"
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm h-full">
                <div class="flex items-center mb-6 pb-4 border-b border-slate-100">
                    ...
                    <h2 class="text-lg font-bold text-slate-900">Daftar Level</h2>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider">
                                ...
                                <th class="p-4 font-semibold border-b border-slate-200">Kelas / Nama</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white text-sm">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="p-4 text-slate-500 text-center font-medium">#{{ $level->id }}</td>
                                ...
                                <td class="p-4">
                                    <div class="flex items-center justify-center space-x-3">
                                        <!-- Edit -->
                                        <a href="{{ route('edit-level', ['id' => $level->id]) }}"
                                        ...
                                        <!-- Delete -->
                                        <form action="{{ route('delete-level', ['id' => $level->id]) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="3"
                                    ...
                                    <p>Belum ada data level tersedia.</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
