            ...
            <div class="row">
                <div class="col-12 col-md-12">
                    <div class="container shadow-lg py-3 my-3 rounded-3">
                        ...
                        <form action="{{ route('update-template', ['id' => $templates->id]) }}" method="POST">
                            @csrf
                            @method('PUT')

                            <!-- Isi formulir edit sesuai dengan kebutuhan aplikasi Anda -->
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput" value="{{ $templates->id }}"
                                    disabled>
                                <label for="floatingInput">ID Template</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput"
                                    value="{{ $templates->nama }}" name="namaBaru" required>
                    ...
                    <button type="button" class="btn btn-sm btn-danger mt-2 mb-3"
                        onclick="location.href='{{ route('admin-addtemplate') }}'">BACK</button>
                </div>
            </div>
        </div>
    </div>
