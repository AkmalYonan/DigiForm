    ...
    <div class="flex justify-center items-center min-h-[60vh]">
        <div
            ...
            <div class="flex flex-col items-center mb-8 pb-6 border-b border-slate-100 text-center">
                <div
                    class="w-16 h-16 rounded-2xl bg-gradient-to-br from-slate-700 to-slate-900 text-white flex items-center justify-center mb-4 shadow-lg shadow-slate-700/30">
                    <i class="fa-solid fa-pen-to-square text-2xl"></i>
                </div>
                <h1 class="text-2xl font-extrabold text-slate-900 display-font">Edit Template</h1>
                <p class="text-slate-500 mt-1 text-sm">Ganti metadata template undangan desain</p>
            </div>

            <form action="{{ route('update-template', ['id' => $templates->id]) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label for="idTemplate" class="block text-sm font-semibold text-slate-700 mb-1">ID Template <span
                            class="text-rose-500">*</span></label>
                    <input type="text" id="idTemplate" value="#{{ $templates->id }}" disabled
                        class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 font-medium cursor-not-allowed">
                </div>

                <div>
                    <label for="namaBaru" class="block text-sm font-semibold text-slate-700 mb-1">Nama Template <span
                            class="text-rose-500">*</span></label>
                    <input type="text" id="namaBaru" name="namaBaru" value="{{ $templates->nama }}" required autofocus
                ...
                <div class="pt-4 flex gap-3">
                    <a href="{{ route('admin-addtemplate') }}"
